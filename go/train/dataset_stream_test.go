...
// TestDatasetStream_BuildDatasetBatches_Good walks the three happy paths of the
// entry point as subtests: the non-packing path (one example per row, grouped
// by BatchSize), the sequence-packing path (rows concatenated up to MaxSeqLen),
// and the free-form text branch combined with zero-batch defaulting.
func TestDatasetStream_BuildDatasetBatches_Good(t *testing.T) {
	...
	t.Run("TextOnlyAndNormalizeBatch", func(t *testing.T) {
		tok := newDatasetStreamBenchTokenizer()
...
// TestDatasetStream_BuildDatasetBatches_Bad asserts the packing-path nil guards:
// a nil tokenizer and a nil dataset are both rejected with an error rather than
// panicking. (The non-packing path delegates to BuildSFTBatches, which has its
// own guards — exercised on both paths here.)
func TestDatasetStream_BuildDatasetBatches_Bad(t *testing.T) {
	tok := newDatasetStreamBenchTokenizer()
...
// TestDatasetStream_BuildDatasetBatches_Ugly covers two degenerate-but-legal
// shapes: an empty dataset (no rows → no batches, no error) and a MaxSeqLen so
// tight (24) that every row is truncated to the cap in datasetPacker.add — the
// aggressive-packing branch the bench also stresses.
func TestDatasetStream_BuildDatasetBatches_Ugly(t *testing.T) {
	tok := newDatasetStreamBenchTokenizer()
