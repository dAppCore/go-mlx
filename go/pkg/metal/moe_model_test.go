...
func TestModel_Generate_SharedMoEFamilies_Good(t *testing.T) {
	...
	// Kimi's + Mixtral's native sparse-expert generate coverage travels with the
	// models in packages metal/model/kimi and metal/model/mixtral; gpt_oss keeps
	// the shared-path coverage in package metal.
	cases := []struct {
		name        string
		modelType   string
