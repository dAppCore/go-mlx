...
// mtp_attn_metal_test.go validates SDPACausalBF16 against the REAL metal.ScaledDotProductAttention
// (the op metal's MTP verify calls). VERIFIED BYTE-IDENTICAL for the self-attention shape qL==kL (any
// GQA) — the path the K-row verify uses when the draft block has no carried cache prefix. For the
// cross-attention shape qL<kL (a draft block over a longer resident cache) there is a single token-
// robust 1-ULP divergence on the full-attention query (the probs·V / softmax accumulation edge metal
// takes for kL>qL); it cannot flip the verify argmax, so the MTP token stream is unaffected. Closing
// that 1-ULP to full hidden-state byte-identity is tracked as the audio-relK-style dispatch match.
// The hermetic scratch-pool/allocation-budget/output-reuse tests for SDPACausalBF16 live in
// mtp_attn_test.go (untagged); sdpaScale is defined there and used by both files.
