...
// normalizeWeightNames strips the gemma4_unified wrapper prefix "language_model." when a real
...
func normalizeWeightNames(tensors map[string]safetensors.Tensor) map[string]safetensors.Tensor {
...
// AssembleGemma4BF16 maps a dense bf16 gemma4 checkpoint onto the native BF16Model through the SAME
...
func AssembleGemma4BF16(tensors map[string]safetensors.Tensor, arch model.Arch) (*BF16Model, error) {
	if arch.HasMoE() {
