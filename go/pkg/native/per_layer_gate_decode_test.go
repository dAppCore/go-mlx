...
func TestGemma4PerLayerGateDecode(t *testing.T) {
	...
	lastHidden := func(withPLE bool) []byte {
		...
		withAutoreleasePool(func() {
			...
			st := newArchDecodeState(arch.Layer, lb, moe, arch.Hidden, arch.Heads, arch.KVHeads, arch.HeadDim, arch.FF, arch.SlidingWindow, arch.RotaryDim, arch.RotaryDimLocal, arch.RopeBase, arch.RopeLocalBase, attnScale, eps)
			if withPLE {
