...
// TestGrpoReward_containsFoldASCII_Ugly exercises containsFoldASCII
// directly — the reward closure that normally calls it short-circuits the
// empty-expected case and gates non-ASCII out beforehand, so these
...
// reward fast path.
func TestGrpoReward_containsFoldASCII_Ugly(t *testing.T) {
...
// TestGrpoReward_grpoRewardStats_Bad covers the zero-length guard in
// grpoRewardStats: with no rollouts the mean and std are both zero rather
// than a divide-by-zero NaN.
func TestGrpoReward_grpoRewardStats_Bad(t *testing.T) {
	mean, std := grpoRewardStats(nil)
	if mean != 0 || std != 0 {
		t.Fatalf("grpoRewardStats(nil) = (%v,%v), want (0,0)", mean, std)
	}
	// A single rollout has zero variance, so the std is exactly zero and
	// the mean is that rollout's reward.
	mean1, std1 := grpoRewardStats([]GRPORollout{{Reward: 1.5}})
	if mean1 != 1.5 || std1 != 0 {
		t.Fatalf("grpoRewardStats(single) = (%v,%v), want (1.5,0)", mean1, std1)
	}
}
