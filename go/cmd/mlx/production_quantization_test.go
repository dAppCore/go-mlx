...
func TestRunCommand_ProductionQuantizationDefaultJSON_Good(t *testing.T) {
	...
	for _, want := range []string{
		...
		`"active_parameter_estimate": 2300000000`,
		`"decode_throughput_estimate": "tok/s ~= measured memory bandwidth bytes/sec / active weight read bytes/token"`,
		`"active_weight_read_bytes_per_token": 2300000000`,
		`"active_weight_read_bytes_per_token": 1725000000`,
		`"active_weight_read_bytes_per_token": 1150000000`,
		`"step_down_to_bits": 6`,
		`"step_down_to_bits": 4`,
		...
		`"combined_mtp_turboquant_policy": {`,
		`"mode": "mtp+turboquant-kv"`,
		`"requires_mtp_promotion": true`,
