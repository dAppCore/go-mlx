...
def build_command(args: argparse.Namespace, paths: dict[str, Path], report_path: Path) -> list[str]:
    return [
        str(args.bin),
        "state-ramp-profile",
        "-json",
...
def run_book(args: argparse.Namespace, command: list[str], run_slug: str) -> int:
    stdout_path = args.run_dir / f"{run_slug}.stdout"
    stderr_path = args.run_dir / f"{run_slug}.stderr"
    env = os.environ.copy()
    if args.metallib:
...
def prepare_book_run(
    ...
    paths = write_materials(args.run_dir, run_slug, seed_prompt, distractors)
    ...
    command_path.write_text(
        json.dumps({"command": command, "random_seed": random_seed}, indent=2) + "\n",
        encoding="utf-8",
    )
    return {
