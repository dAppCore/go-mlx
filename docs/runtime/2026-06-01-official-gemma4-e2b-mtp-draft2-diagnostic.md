...
10-turn workflow, does not cover the required `1,2,4` draft-token sweep, and
does not promote MTP as a default.
...
- target verify calls: `90`
- draft calls: `90`
- target verify throughput: `28.190787594849148 tok/s`
- warm decode throughput: `26.88335766075169 tok/s`
...
runtime emits the required MTP counters, but draft-2 on this short official
source-snapshot prompt is slower than target-only, accepts too few draft tokens,
and does not satisfy greedy output parity. The next MTP benchmark must use the
retained workflow shape from `GOAL.md`: at least `10` turns, real restore
durations, and the `1,2,4` draft-token sweep before any default decision.
