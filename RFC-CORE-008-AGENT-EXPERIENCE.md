...
**Audit cadence:** Quarterly P6 compliance review across Go lib tier; failures trigger child-tickets per module.
...
### 11. Benchmarks as Hot-Path Validation

Performance is correctness in the AX era. Agents have the patience to iterate optimisation passes that humans don't — provided the measurement instrument exists. Hot-path functions ship with benchmarks so per-call cost is visible without reading the function body.

```go
// BenchmarkPhoneticReach_BlockedTopics measures the cross-product scan
// PhoneticReach does over (tokens × topics). Pre-computed topic codes
// dropped this from 88,687 ns/op (3550 allocs) to 1,734 ns/op (51 allocs)
// — a 51x speedup found purely by reading the benchmark output.
...
**Rule:** Functions called per-token, per-record, per-request, or per-frame — or sitting on the public API of a frequently-imported package — ship with one or more `BenchmarkFunctionName_Scenario` tests. Each benchmark calls `b.ReportAllocs()` so allocation count is visible alongside ns/op.

**Rationale:** Sub-optimal paths are mechanically findable when benchmarks exist. The discovery loop is:

1. Run `go test -bench=. -benchmem ./...`
2. Spot the high-allocs-per-op or high-ns-per-op entry
3. Profile the function (`go test -bench=Foo -cpuprofile=cpu.prof`) or read it
4. Identify redundant work (re-encoding, re-tokenising, re-allocating in a loop)
5. Optimise + re-run benchmark to verify the improvement

Without benchmarks, the performance gap is invisible until production. With benchmarks, an agent finds the 50x win in the first sweep — the [[research-lek-artifact-phonetic-circumvention]] `PhoneticReach` case landed exactly this way during the U lane: the benchmark immediately surfaced "3550 allocs per call" as out-of-band, and the fix (pre-compute topic codes once) was a 30-minute edit.

**Corollary — benchmark output is a contract:** A future agent can grep `BenchmarkImprint_FullResponse` and see "~125μs / 1814 allocs / 137KB" as the documented expected cost. Regression = a future PR slows the function below the baseline. CI failure on regression is then mechanical, not subjective.

**Anti-pattern:** "We'll add benchmarks later when we optimise." Performance work without benchmarks is guessing. The benchmark IS the optimisation work — write it first, then iterate against the measured baseline. The agent's first pass on a hot path is "add the benchmark"; the second pass is "fix what the benchmark surfaced."

**Audit cadence:** Per-PR review includes `go test -bench=. -benchmem` output for any touched hot-path package. New hot-path functions without accompanying benchmarks block merge. Alpha.1 graduation requires non-trivial hot-path coverage.

**What counts as a hot path:**

| Hot — ships with benchmark | Not hot — benchmark optional |
|---|---|
| Per-token scoring (`Score`, `Imprint`, `DoubleMetaphone`) | Init / boot-time setup (`NewService`, `Register`) |
| Per-record fingerprinting (anything called inside `runProbe`) | One-shot CLI dispatch |
| Per-request middleware | Helper functions called once per program run |
| Per-frame render logic | Build-time code generation |
| Public API of a frequently-imported lib package (`core.JSONMarshal`, `paths.AtomicWrite`) | Internal one-off utilities not on a hot path |
| Cross-product algorithms (O(n×m) or worse) | Trivial getters/setters |

**Naming convention:** `Benchmark<FunctionName>_<Scenario>`. The scenario suffix documents what input shape is being measured — `_FullResponse`, `_SingleWord`, `_TenLines`, `_BlockedTopics`, `_EmptyInput`. Multiple scenarios per function are encouraged when the function's cost varies with input shape.

**Cross-language:** This principle is Go-specific in syntax (`testing.B`, `b.N`, `b.ReportAllocs()`) but applies in spirit to PHP (PHPBench), TS (vitest's bench mode), and any other language tier in the ecosystem. Each language tier defines its own equivalent benchmark surface; the discipline is identical — hot paths measured, allocations tracked, regressions caught.
...
Project-tier and code-tier RFCs must declare implementation truth before making implementation claims. Agents and humans must grep for `Status:` before trusting prose written in the present tense.

Every `project/**/RFC.md`, `project/**/RFC.*.md`, `code/**/RFC.md`, and `code/**/RFC.*.md` file carries a top-of-file frontmatter marker:
...
Done: API contracts and storage models are implemented.
Pending: Admin UI and end-to-end deployment verification remain.
```
...
| Status | Meaning |
|--------|---------|
| `Implemented` | The RFC is shipped end to end and can be treated as current implementation truth. |
| `Partial` | Some of the RFC is shipped, but the document still contains unshipped claims. `Done:` and `Pending:` lines are required. |
| `Planned` | The RFC is scoped and intended, but not yet shipped. |
| `Aspirational` | The RFC records a vision, strategy, or possibility that is not yet implementation-backed. |

RFCs describe what the system IS — present tense, no status overrides, no scheduling. Status, version, and staging are scheduling concerns and belong in the tracker (Mantis), never in the spec. Sub-agents reading a spec that calls part of itself "Planned" or "Deferred" will silently omit those sections from feature implementation. The cost of staging-language in specs is integration-time discovery of gaps that should have been in scope.

If a section describes a contract the system honours when a precondition exists, write the precondition into the contract:

```markdown
## 4. Provider Registry

Provider registry binds to runtime when at least one Provider has registered.
The Provider interface and six provider packages define the registration shape.
Runtime registration wires through Service.Register; production gateway wiring
consumes the registry via core.Action("provider.registry.get").
```

That sentence is true today; it stays true when more providers register. No staging required.
...
- 2026-05-21: Principle 11 added — Benchmarks as Hot-Path Validation. Triggered by a 51x speedup on `contentshield.PhoneticReach` found by reading benchmark output: pre-computed topic Metaphone codes once outside the per-token loop. The PR-review-incident becomes the standard: hot paths ship with `Benchmark*` functions + `b.ReportAllocs()`, and sub-optimal paths surface mechanically.
