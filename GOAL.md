...
| Current q6 go-mlx self-benchmark refresh | The fresh rebuilt `lthn-mlx` q6 E2B self-benchmark compares current defaults against the same binary with `-fast-gemma4-lane=false`, using `context=4096`, `max_tokens=512`, `runs=3`, hidden output, and a `75 W` normalised energy estimate. The default gate set (`GO_MLX_ENABLE_DIRECT_GREEDY_TOKEN=1`, `GO_MLX_ENABLE_PAGED_DECODE_FAST_CONCAT=1`) records `1164` generated tokens, `89.49273128756029 tok/s`, `13.213508334s` wall, `991.01312505 J`, `4165910528` B peak RSS, and output hash `ea621e942f414fde824380a89a39cd120283fe303e34a5930b7a046c950a6754`. The same-binary baseline records the same `1164` tokens and output hash at `89.11955735839634 tok/s`, `13.266706126s`, `995.00295945 J`, and `4222173184` B peak RSS. This keeps the default as a small positive go-mlx-vs-go-mlx regression guard (`1.0042x` decode, `0.40%` lower wall/energy), not an external parity claim. See `docs/runtime/2026-06-01-gemma4-e2b-q6-go-mlx-self-bench.md` |
